...
def index(request):
    ...
    through_widget = GetHtmlEntityIdSelector(request, "through", cids_or_cnames_list=["connection_type"], submit_form_on_select=False)
    ...
    for conn in connections_list:
        t_conn = {
            "cid" : conn.cid,
            "id" : conn.id,
            "from" : getFirstOrNone(conn.getNeighboursFrom(filter_func=getFilterNeighboursByClassName(request.configuration, "device"))),
            "through" : getFirstOrNone(conn.getNeighbours(filter_func=getFilterNeighboursByClassName(request.configuration, "connection_type"))),
            "to" : getFirstOrNone(conn.getNeighboursTo(filter_func=getFilterNeighboursByClassName(request.configuration, "device"))),
    ...
    return {"from_widget" : from_widget, "through_widget" : through_widget, "to_widget" : to_widget, "connections" : connections}
