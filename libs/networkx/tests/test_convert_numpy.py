...
class TestConvertNumpy(object):
    ...
    def test_identity_graph_matrix(self):
        "Conversion from nxgraph to matrix to nxgraph."
        A = nx.to_numpy_matrix(self.G1)
        self.identity_conversion(self.G1, A, nx.Graph())

    def test_identity_graph_array(self):
        "Conversion from nxgraph to array to nxgraph."
        A = nx.to_numpy_matrix(self.G1)
        A = np.asarray(A)
        self.identity_conversion(self.G1, A, nx.Graph())
    ...
    def test_identity_weighted_graph_matrix(self):
        """Conversion from weighted nxgraph to matrix to weighted nxgraph."""
        A = nx.to_numpy_matrix(self.G3)
        self.identity_conversion(self.G3, A, nx.Graph())

    def test_identity_weighted_graph_array(self):
        """Conversion from weighted nxgraph to array to weighted nxgraph."""
        A = nx.to_numpy_matrix(self.G3)
        A = np.asarray(A)
        self.identity_conversion(self.G3, A, nx.Graph())
    ...
    def test_nodelist(self):
        """Conversion from nxgraph to matrix to nxgraph with nodelist."""
        P4 = path_graph(4)
        P3 = path_graph(3)
        nodelist = P3.nodes()
