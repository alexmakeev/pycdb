...
def full_rary_tree(r, n, create_using=None):
    ...
    create_using : NetworkX nxgraph type, optional
        Use specified type to construct nxgraph (default = networkx.Graph)
...
def barbell_graph(m1,m2,create_using=None):
    ...
    This nxgraph is an extremal example in David Aldous
    ...
    if m1<2:
        raise nx.NetworkXError(\
              "Invalid nxgraph description, m1 should be >=2")
    if m2<0:
        raise nx.NetworkXError(\
              "Invalid nxgraph description, m2 should be >=0")
...
def complete_graph(n,create_using=None):
    """ Return the complete nxgraph K_n with n nodes.
...
def complete_bipartite_graph(n1,n2,create_using=None):
    """Return the complete bipartite nxgraph K_{n1_n2}.
...
def circular_ladder_graph(n,create_using=None):
    """Return the circular ladder nxgraph CL_n of length n.
...
def cycle_graph(n,create_using=None):
    """Return the cycle nxgraph C_n over n nodes.
...
def dorogovtsev_goltsev_mendes_graph(n,create_using=None):
    """Return the hierarchically constructed Dorogovtsev-Goltsev-Mendes nxgraph.
...
def empty_graph(n=0,create_using=None):
    """Return the empty nxgraph with n nodes and zero edges.
    ...
    The variable create_using should point to a "nxgraph"-like object that
    will be cleaned (nodes and edges will be removed) and refitted as
    an empty "nxgraph" with n nodes with integer labels. This capability
    is useful for specifying the class-nature of the resulting empty
    "nxgraph" (i.e. Graph, DiGraph, MyWeirdGraphClass, etc.).
    ...
    Secondly, one can pass an existing nxgraph (digraph, pseudograph,
    etc.) via create_using. For example, if G is an existing nxgraph
    ...
    nxgraph (resp. digraph, pseudograph, etc.).

    See also create_empty_copy(G).
    
    """
    if create_using is None:
        # default empty nxgraph is a simple nxgraph
        G=nx.Graph()
    else:
        G=create_using
        G.clear()
...
def grid_2d_graph(m,n,periodic=False,create_using=None):
    """ Return the 2d grid nxgraph of mxn nodes,
...
def grid_graph(dim,periodic=False):
    ...
    while len(dim)>0:
        ...
        # This is so that we get a new nxgraph of Gnew's class.
        G=nx.cartesian_product(Gnew,Gold)
    # nxgraph G is done but has labels of the form (1,(2,(3,1)))
    # so relabel
...
def ladder_graph(n,create_using=None):
    """Return the Ladder nxgraph of length n.
...
def lollipop_graph(m,n,create_using=None):
    ...
    For m>1 and n>=0, the complete nxgraph K_m is connected to the
    path P_n.  The resulting m+n nodes are labelled 0,...,m-1 for the
    complete nxgraph and m,...,m+n-1 for the path. The 2 subgraphs
    are joined via the edge (m-1,m).  If n=0, this is merely a complete 
    nxgraph.

    Node labels are the integers 0 to number_of_nodes - 1.

    (This nxgraph is an extremal example in David Aldous and Jim
    ...
    if m<2:
        raise nx.NetworkXError(\
              "Invalid nxgraph description, m should be >=2")
    if n<0:
        raise nx.NetworkXError(\
              "Invalid nxgraph description, n should be >=0")
    # the ball
...
def null_graph(create_using=None):
    """ Return the Null nxgraph with no nodes or edges.
...
def path_graph(n,create_using=None):
    """Return the Path nxgraph P_n of n nodes linearly connected by n-1 edges.
...
def star_graph(n,create_using=None):
    """ Return the Star nxgraph with n+1 nodes: one center node, connected to n outer nodes.
...
def trivial_graph(create_using=None):
    """ Return the Trivial nxgraph with one node (with integer label 0) and no edges.
...
def wheel_graph(n,create_using=None):
    """ Return the wheel nxgraph: a single hub node connected to each node of the (n-1)-node cycle nxgraph.
