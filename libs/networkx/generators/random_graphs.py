...
def fast_gnp_random_graph(n, p, seed=None, directed=False):
    """Return a random nxgraph G_{n,p} (Erdős-Rényi nxgraph, binomial nxgraph).
    ...
    Notes
    -----
    The G_{n,p} nxgraph algorithm chooses each of the [n(n-1)]/2
    ...
    the expected number of edges is small (sparse nxgraph).
    ...
    v = 1  # Nodes in nxgraph are from 0,n-1 (this is the second node index).
    w = -1
    lp = math.log(1.0 - p)  
...
def dense_gnm_random_graph(n, m, seed=None):
    """Return the random nxgraph G_{n,m}.

    Gives a nxgraph picked randomly out of the set of all graphs
...
def gnm_random_graph(n, m, seed=None, directed=False):
    ...
    Produces a nxgraph picked randomly out of the set of all graphs
    with n nodes and m edges.
...
def newman_watts_strogatz_graph(n, k, p, seed=None):
    """Return a Newman-Watts-Strogatz small world nxgraph.
...
def watts_strogatz_graph(n, k, p, seed=None):
    """Return a Watts-Strogatz small-world nxgraph.
    ...
    rewiring does not increase the number of edges. The rewired nxgraph
...
def connected_watts_strogatz_graph(n, k, p, tries=100, seed=None):
    """Return a connected Watts-Strogatz small-world nxgraph.
    ...
    p : float 
        The probability of rewiring each edge 
    tries : int
        Number of attempts to generate a connected nxgraph.
    seed : int, optional
...
def random_regular_graph(d, n, seed=None):
    """Return a random regular nxgraph of n nodes each with degree d.
    
    The resulting nxgraph G has no self-loops or parallel edges.
    ...
    # If False, the generation of the nxgraph has failed
...
def barabasi_albert_graph(n, m, seed=None):
    """Return random nxgraph using Barabási-Albert preferential attachment model.
        
    A nxgraph of n nodes is grown by attaching new nodes each with m
    ...
    Notes
    -----
    The initialization is a nxgraph with with m nodes and no edges.
...
def powerlaw_cluster_graph(n, m, p, seed=None):
    ...
    It seems possible to have a disconnected nxgraph with this algorithm
...
def random_lobster(n, p1, p2, seed=None):
    """Return a random lobster.
     ...
     A caterpillar is a tree that reduces to a path nxgraph when pruning
     all leaf nodes (p2=0).
    ...
    seed : int, optional
        Seed for random number generator (default=None).   
    """
    # a necessary ingredient in any self-respecting nxgraph library
    if seed is not None:
        random.seed(seed)
    llen=int(2*random.random()*n + 0.5)
    L=path_graph(llen)
    L.name="random_lobster(%d,%s,%s)"%(n,p1,p2)
    # build caterpillar: add edges to path nxgraph with probability p1
    current_node=llen-1
    for n in range(llen):
...
def random_shell_graph(constructor, seed=None):
    """Return a random shell nxgraph for the constructor given.
