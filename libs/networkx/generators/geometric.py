...
def random_geometric_graph(n, radius, dim=2, pos=None):
    r"""Return the random geometric nxgraph in the unit cube.

    The random geometric nxgraph model places n nodes uniformly at random
    ...
    Notes
    -----
    This uses an `n^2` algorithm to build the nxgraph.  A faster algorithm
...
def geographical_threshold_graph(n, theta, alpha=2, dim=2, 
    ...
    r"""Return a geographical threshold nxgraph.

    The geographical threshold nxgraph model places n nodes uniformly at random
...
def geographical_threshold_edges(G, theta, alpha=2):
    # generate edges for a geographical threshold nxgraph given a nxgraph
...
def waxman_graph(n, alpha=0.4, beta=0.1, L=None, domain=(0,0,1,1)):
    r"""Return a Waxman random nxgraph.

    The Waxman random nxgraph models place n nodes uniformly at random
    ...
    Waxman-1:  `L` not specified
       ...
       `L` is the maximum distance between all nodes in the nxgraph.
    ...
    """
    # build nxgraph of n nodes with random positions in the unit square
    G = nx.Graph()
    G.add_nodes_from(range(n))
...
def navigable_small_world_graph(n, p=1, q=1, r=2, dim=2, seed=None):
    r"""Return a navigable small-world nxgraph.

    A navigable small-world nxgraph is a directed grid with additional
