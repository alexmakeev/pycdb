...
def is_threshold_graph(G):
    """
    Returns True if G is a threshold nxgraph.
    """
...
def is_threshold_sequence(degree_sequence):
    ...
    Uses the property that a threshold nxgraph must be constructed by
...
def uncompact(creation_sequence):
    ...
    nxgraph to a standard creation sequence (unlabeled).
...
def creation_sequence_to_weights(creation_sequence):
    ...
    nxgraph designated by the creation sequence.  The weights
...
def weights_to_creation_sequence(weights, threshold=1, with_labels=False, compact=False):
    """
    Returns a creation sequence for a threshold nxgraph
...
def threshold_graph(creation_sequence, create_using=None):
    """
    Create a threshold nxgraph from the creation sequence or compact
    creation_sequence.
...
def find_threshold_graph(G, create_using=None):
    ...
    The threshold nxgraph will contain the largest degree node in G.
...
def find_creation_sequence(G):
    ...
    Returns the labeled creation sequence of that threshold nxgraph.
    """
    cs = []
    # get a local pointer to the working part of the nxgraph
    H = G
    while H.order() > 0:
        ...
        ds.sort()
        # Update threshold nxgraph nodes
...
def triangles(creation_sequence):
    """
    Compute number of triangles in the threshold nxgraph with the
...
def triangle_sequence(creation_sequence):
    """
    Return triangle sequence for the given threshold nxgraph creation sequence.
...
def cluster_sequence(creation_sequence):
    """
    Return cluster sequence for the given threshold nxgraph creation sequence.
    """
...
def degree_sequence(creation_sequence):
    """
    Return degree sequence for the threshold nxgraph with the given
    creation sequence
    """
    cs = creation_sequence  # alias
    seq = []
...
def density(creation_sequence):
    """
    Return the density of the nxgraph with this creation_sequence.
...
def shortest_path(creation_sequence, u, v):
    ...
    threshold nxgraph G with the given creation_sequence.
    ...
    if v not in verts:
        raise ValueError("Vertex %s not in nxgraph from creation_sequence" % v)
    if u not in verts:
        raise ValueError("Vertex %s not in nxgraph from creation_sequence" % u)
    # Done checking
    if u == v: return [u]
...
def shortest_path_length(creation_sequence, i):
    ...
    every other node for the threshold nxgraph with the given
    creation sequence.
...
def betweenness_sequence(creation_sequence, normalized=True):
    """
    Return betweenness for the threshold nxgraph with the given creation
...
def eigenvalues(creation_sequence):
    ...
    nxgraph for the given creation_sequence.
...
### Threshold nxgraph creation routines
...
def random_threshold_sequence(n, p, seed=None):
    ...
    A "random" threshold nxgraph can be built with
...
def swap_d(cs, p_split=1.0, p_combine=1.0, seed=None):
    ...
    in the nxgraph for the given sequence.
    The resulting sequence is still a threshold sequence.

    Perform one split and one combine operation on the
    'd's of a creation sequence for a threshold nxgraph.
    This operation maintains the number of nodes and edges
    in the nxgraph, but shifts the edges from node to node
    maintaining the threshold quality of the nxgraph.
    """
    if not seed is None:
        random.seed(seed)
