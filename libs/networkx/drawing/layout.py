...
def shell_layout(G,nlist=None,dim=2,scale=1):
    ...
    if len(G)==1:
        return {G.nodes()[0]:(1,)*dim}
    if nlist==None:
        nlist=[G.nodes()] # draw the whole nxgraph in one shell
...
def _fruchterman_reingold(A, dim=2, pos=None, fixed=None, iterations=50):
    ...
    # Entry point for NetworkX nxgraph is fruchterman_reingold_layout()
    try:
        import numpy as np
    except ImportError:
...
def spectral_layout(G, dim=2, weight='weight', scale=1):
    """Position nodes using the eigenvectors of the nxgraph Laplacian.
...
def _sparse_spectral(A,dim=2):
    ...
    # Could use multilevel methods here, see Koren "On spectral nxgraph drawing"
    try:
        import numpy as np
