...
class BaseGraphTester(object):
    """ Tests for data-structure independent nxgraph class features."""
    ...
    def test_nbunch_iter(self):
        ...
        # sequence with none in nxgraph
        assert_equal(list(G.nbunch_iter([-1])),[])
        # string sequence with none in nxgraph
        assert_equal(list(G.nbunch_iter("foo")),[])
        # node not in nxgraph doesn't get caught upon creation of iterator
...
class BaseAttrGraphTester(BaseGraphTester):
    """ Tests of nxgraph class attribute features."""
    def test_weighted_degree(self):
        G=self.Graph()
...
class TestGraph(BaseAttrGraphTester):
    """Tests specific to dict-of-dict-of-dict nxgraph data structure"""
    def setUp(self):
