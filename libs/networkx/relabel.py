...
def relabel_nodes(G, mapping, copy=True):
    """Relabel the nodes of the nxgraph G.
    ...
    The keyword setting copy=False modifies the nxgraph in place.
...
def _relabel_inplace(G, mapping):
    ...
    for old in nodes:
        ...
        try:
            ...
            raise KeyError("Node %s is not in the nxgraph" % old)
        if multigraph:
...
def convert_node_labels_to_integers(G, first_label=0, ordering="default",
    ...
    #    edges of a nxgraph, and returns a nxgraph with appropriate integer
    #    labels. One can view this as a re-labeling of the nodes. Be
    #    warned that the term "labeled nxgraph" has a loaded meaning
    #    in nxgraph theory. The fundamental issue is whether the names
    #    (labels) of the nodes (and edges) matter in deciding when two
    #    graphs are the same. For example, in problems of nxgraph enumeration
    #    there is a distinct difference in techniques required when
    #    counting labeled vs. unlabeled graphs.

    #    When implementing nxgraph
    ...
    #    returned but made an attribute of the new nxgraph.
