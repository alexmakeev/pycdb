...
def is_biconnected(G):
    """Return True if the nxgraph is biconnected, False otherwise.

    A nxgraph is biconnected if, and only if, it cannot be disconnected by
    removing only one node (and all edges incident on that node). If
    removing a node increases the number of disconnected components
    in the nxgraph, that node is called an articulation point, or cut
    vertex.  A biconnected nxgraph has no articulation points.

    Parameters
    ----------
    G : NetworkX Graph
        An undirected nxgraph.

    Returns
    -------
    biconnected : bool
        True if the nxgraph is biconnected, False otherwise.

    Raises
    ------
    NetworkXError :
        If the input nxgraph is not undirected.
    ...
    .. [1] Hopcroft, J.; Tarjan, R. (1973). 
       "Efficient algorithms for nxgraph manipulation".
       Communications of the ACM 16: 372–378. doi:10.1145/362248.362272
    """
    bcc = list(biconnected_components(G))
    if not bcc:  # No bicomponents (it could be an empty nxgraph)
        return False
...
def biconnected_component_edges(G):
    ...
    component of the input nxgraph.
...
def biconnected_component_subgraphs(G):
    """Return a generator of graphs, one nxgraph for each biconnected component
    of the input nxgraph.
    ...
    -------
    graphs : generator
        Generator of graphs, one nxgraph for each biconnected component.
...
def articulation_points(G):
    """Return a generator of articulation points, or cut vertices, of a nxgraph.

    An articulation point or cut vertex is any node whose removal (along with
    all its incident edges) increases the number of connected components of 
    a nxgraph. An undirected connected nxgraph without articulation points is
    biconnected. Articulation points belong to more than one biconnected
    component of a nxgraph.
