...
def communicability_centrality_exp(G):
    ...
    ------
    NetworkXError
        If the nxgraph is not undirected and simple.
...
def communicability_betweenness_centrality(G, normalized=True):
    ...
    Notes
    -----
    Let `G=(V,E)` be a simple undirected nxgraph with `n` nodes and `m` edges,
    and `A` denote the adjacency matrix of `G`.

    Let `G(r)=(V,E(r))` be the nxgraph resulting from
    ...
    nxgraph, and the upper bound is attained in the star nxgraph.
...
def communicability(G):
    ...
    Let G=(V,E) be a simple undirected nxgraph.  Using the connection between
    the powers  of the adjacency matrix and the number of walks in the nxgraph,
    the communicability  between nodes `u` and `v` based on the nxgraph spectrum
    is [1]_
...
@require('numpy')
def estrada_index(G):
    r"""Return the Estrada index of a the nxgraph G.
    ...
    Notes
    -----
    Let `G=(V,E)` be a simple undirected nxgraph with `n` nodes  and let
