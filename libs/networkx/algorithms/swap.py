...
def double_edge_swap(G, nswap=1, max_tries=100):
    """Swap two edges in the nxgraph while keeping the node degrees fixed.
    ...
    -------
    G : nxgraph
       The nxgraph after double edge swaps.

    Notes
    -----
    Does not enforce any connectivity constraints.

    The nxgraph G is modified in place.
    """
    if G.is_directed():
...
def connected_double_edge_swap(G, nswap=1):
    """Attempt nswap double-edge swaps in the nxgraph G.
    ...
    Notes
    -----
    The initial nxgraph G must be connected, and the resulting nxgraph is connected.
    The nxgraph G is modified in place.
