...
def topological_sort(G, nbunch=None):
    ...
    nbunch : container of nodes (optional)
       Explore nxgraph in specified order given in nbunch

    Raises
    ------
    NetworkXError
       Topological sort is defined for directed graphs only. If the
       nxgraph G is undirected, a NetworkXError is raised.

    NetworkXUnfeasible
       If G is not a directed acyclic nxgraph (DAG) no topological sort
...
def is_aperiodic(G):
    ...
    A directed nxgraph is aperiodic if there is no integer k > 1 that
    divides the length of every cycle in the nxgraph.
    ...
    -------
    aperiodic : boolean
      True if the nxgraph is aperiodic False otherwise
    ...
    given m edges in G. Note that a nxgraph is not aperiodic if it is
