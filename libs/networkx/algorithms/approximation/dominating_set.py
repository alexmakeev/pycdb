...
A dominating set for a nxgraph G = (V, E) is a subset D of V such that every
...
set for G. Given a nxgraph G = (V, E) find a minimum weight dominating set V'.
...
def min_weighted_dominating_set(graph, weight=None):
    ...
    """
    if not graph:
        raise ValueError("Expected non-empty NetworkX nxgraph!")
