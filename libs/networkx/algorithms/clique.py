...
def find_cliques(G):
    """Search for all maximal cliques in a nxgraph.
    ...
    .. [1] Bron, C. and Kerbosch, J. 1973.
       Algorithm 457: finding all cliques of an undirected nxgraph.
    ...
    maxconn = -1
    nnbrs = {}
    pivotnbrs = set()  # handle empty nxgraph
...
def find_cliques_recursive(G):
    """Recursive search for all maximal cliques in a nxgraph.
    ...
    for n, nbrs in G.adjacency_iter():
        ...
        nbrs.discard(n)
        nnbrs[n] = nbrs
    if not nnbrs: return []  # empty nxgraph
    cand = set(nnbrs)
    done = set()
    clique_so_far = []
    cliques = []
...
def make_clique_bipartite(G, fpos=None, create_using=None, name=None):
    """Create a bipartite clique nxgraph from a nxgraph G.
...
def project_down(B, create_using=None, name=None):
    """Project a bipartite nxgraph B down onto its "bottom nodes".

    The nodes retain their names and are connected if they
    share a common top node in the bipartite nxgraph.
...
def node_clique_number(G, nodes=None, cliques=None):
    ...
    if cliques is None:
        if nodes is not None:
            # Use ego_graph to decrease size of nxgraph
    ...
    if nodes is None:
        nodes = G.nodes()  # none, get entire nxgraph
    ...
    # if nodes is None:                 # none, use entire nxgraph
    #     nodes=G.nodes()
