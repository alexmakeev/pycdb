...
def blockmodel(G, partitions, multigraph=False):
    ...
    (block) is represented as a single node in the reduced nxgraph.

    Edges between nodes in the block nxgraph are added according to the
    edges in the original nxgraph.  If the parameter multigraph is False
    (the default) a single edge is added with a weight equal to the
    sum of the edge weights between nodes in the original nxgraph
    ...
    with the edge data from the original nxgraph.
    ...
    multigraph : bool, optional
        ...
        nxgraph applied to each corresponding edge in the new nxgraph.
        If False return a Graph with the sum of the edge weights, or a
        count of the edges if the original nxgraph is unweighted.

    Returns
    -------
    blockmodel : a Networkx nxgraph object
