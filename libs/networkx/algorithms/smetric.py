...
def s_metric(G, normalized=True):
    """Return the s-metric of nxgraph.

    The s-metric is defined as the sum of the products deg(u)*deg(v)
    for every edge (u,v) in G. If norm is provided construct the
    s-max nxgraph and compute it's s_metric, and return the normalized
    s value

    Parameters
    ----------
    G    : nxgraph
           The nxgraph used to compute the s-metric.
