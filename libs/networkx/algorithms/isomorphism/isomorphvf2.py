...
An implementation of VF2 algorithm for nxgraph ismorphism testing.
...
are also possible.  For example, a subgraph of one nxgraph
can be checked for isomorphism to a second nxgraph.
...
In the VF2 literature, a mapping M is said to be a nxgraph-subgraph
isomorphism iff M is an isomorphism between G2 and a subgraph of G1.
Thus, to say that G1 and G2 are nxgraph-subgraph isomorphic is to say
...
Let G=(N,E) be a nxgraph with a set of nodes N and set of edges E.
...
[2]   L. P. Cordella, P. Foggia, C. Sansone, M. Vento, "An Improved
      ...
      pp. 149-159, 2001.
      http://amalfi.dis.unina.it/nxgraph/db/papers/vf-algorithm.pdf
...
class GraphMatcher(object):
    ...
    def __init__(self, G1, G2):
        ...
        # Declare that we will be searching for a nxgraph-nxgraph isomorphism.
        self.test = 'nxgraph'
    ...
    def isomorphisms_iter(self):
        ...
        # Declare that we are looking for a nxgraph-nxgraph isomorphism.
        self.test = 'nxgraph'
        self.initialize()
    ...
    def semantic_feasibility(self, G1_node, G2_node):
        ...
        tests::
          ...
          test='nxgraph'
            Indicates that the nxgraph matcher is looking for a nxgraph-nxgraph
            isomorphism.

          test='subgraph'
            Indicates that the nxgraph matcher is looking for a subgraph-nxgraph
    ...
    def subgraph_isomorphisms_iter(self):
        ...
        # Declare that we are looking for nxgraph-subgraph isomorphism.
        self.test = 'subgraph'
        self.initialize()
        for mapping in self.match():
            yield mapping
        ...
        for neighbor in self.G2[G2_node]:
            if neighbor in self.core_2:
                if not (self.core_2[neighbor] in self.G1[G1_node]):
                    return False
                elif self.G1.number_of_edges(self.core_2[neighbor], G1_node) != self.G2.number_of_edges(neighbor,
                                                                                                        G2_node):
                    return False
        ...
        for successor in self.G2[G2_node]:
            if successor in self.core_2:
                if not (self.core_2[successor] in self.G1[G1_node]):
                    return False
                elif self.G1.number_of_edges(G1_node, self.core_2[successor]) != self.G2.number_of_edges(G2_node,
                                                                                                         successor):
                    return False
...
class DiGMState(object):
    ...
    def __init__(self, GM, G1_node=None, G2_node=None):
        ...
        if G1_node is not None and G2_node is not None:
            ...
            for node in GM.core_1:
                new_nodes.update(
                    [predecessor for predecessor in GM.G1.predecessors(node) if predecessor not in GM.core_1])
            for node in new_nodes:
            ...
            for node in GM.core_2:
                new_nodes.update(
                    [predecessor for predecessor in GM.G2.predecessors(node) if predecessor not in GM.core_2])
            for node in new_nodes:
