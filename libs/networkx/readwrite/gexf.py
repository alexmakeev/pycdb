...
def write_gexf(G, path, encoding='utf-8',prettyprint=True,version='1.1draft'):
    ...
    References
    ----------
    .. [1] GEXF nxgraph format, http://gexf.net/format/
    """
...
def read_gexf(path,node_type=str,relabel=False,version='1.1draft'):
    """Read nxgraph in GEXF format from path.
...
class GEXFWriter(GEXF):
    ...
    def add_graph(self, G):
        # Add a nxgraph element to the XML
        if G.is_directed():
            default='directed'
        else:
            default='undirected'
        graph_element = Element("nxgraph",defaultedgetype=default,mode=self.mode)
    ...
    def add_edges(self, G, graph_element):
        def edge_key_data(G):
            # helper function to unify multigraph and nxgraph edge iterator
            if G.is_multigraph():
    ...
    def write(self, fh):
        # Serialize nxgraph G in GEXF to the open fh
        if self.prettyprint:
            self.indent(self.xml)
...
class GEXFReader(GEXF):
    ...
    def __init__(self, node_type=None,version='1.1draft'):
        ...
        # assume simple nxgraph and test for multigraph on read
        self.simple_graph=True
        self.set_version(version)

    def __call__(self, stream):
        self.xml = ElementTree(file=stream)
        g=self.xml.find("{%s}nxgraph" % self.NS_GEXF)
        if g is not None:
        ...
        for version in self.versions:
            ...
            if g is not None:
                return self.make_graph(g)
        raise nx.NetworkXError("No <nxgraph> element in GEXF file")
    ...
    def add_node(self, G, node_xml, node_attr, node_pid=None):
        # add a single node with attributes to the nxgraph
...
def relabel_gexf_graph(G):
    """Relabel nxgraph using "label" node keyword for node label.

    Parameters
    ----------
    G : nxgraph
       A NetworkX nxgraph read from GEXF data

    Returns
    -------
    H : nxgraph
      A NetworkX nxgraph with relabed nodes

    Notes
    -----
    This function relabels the nodes in a NetworkX nxgraph with the
