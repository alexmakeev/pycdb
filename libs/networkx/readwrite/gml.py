...
def read_gml(path,encoding='UTF-8',relabel=False):
    """Read nxgraph in GML format from path.
...
def parse_gml(lines, relabel=True):
    """Parse GML nxgraph from a string or iterable.
    ...
    NetworkX nxgraph, node, and edge attribute structures.
...
def write_gml(G, path):
    """
    Write the nxgraph G in GML format to the file or file handle path.
