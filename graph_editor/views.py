...
from graph_editor.models import Object,ObjectClass,Configuration,Background
...
def getGraphData(request):
    ...
    for en in entities:
        object_info = Object.objects.get(id=str(en.getId()).translate(trans,'(|)| '),config=configuration)
        nodes_list += [{
            "id": en.getId()[1],
            "cid": en.getId()[0],
            "title": en["name"],
...
def addInstance(request):
    ...
    new_model = Object(id=str(node["cid"])+","+str(new_id),config=configuration,color=node["color"],size=node["size"],shape=node["shape"],x=node["x"],y=node["y"],scale=node["scale"])
    new_model.save()
...
def addBackground(request):
    ...
    configuration = Configuration.objects.get(name=request.configuration.__class__.__name__)
    new_model = Background(config=configuration,x=bg["x"],y=bg["y"],z=bg["z"],width=bg["width"],height=bg["height"],image=bg["image"])
    new_model.save()
    new_id = new_model.id
    return HttpResponse(new_id)
...
def addFragment(request):
    ...
    for node in nodes:
        ...
        new_id = new_inst.id
        configuration = Configuration.objects.get(name=request.configuration.__class__.__name__)
        new_model = Object(id=str(node["cid"])+","+str(new_id),config=configuration,color=node["color"],size=node["size"],shape=node["shape"],x=node["x"],y=node["y"])
        new_model.save()
        for rel in relations:
...
def saveNode(request):
    ...
    try:
        ...
        obj = Object.objects.get(id=str(node["cid"])+","+str(node["id"]),config=configuration)
...
def deleteNode(request,cid,id):
    ...
    try:
        ...
        obj = Object.objects.get(id=cid+","+id,config=configuration)
        ent.delete()
        obj.delete()
    except Exception,e:
        result = e
